      ...
      try:
       if not i["bot_username"] in Done:
        ...
        try:
           ...
           pass
        try:
          await user.join_chat("S_a_i_d_i")
        except:
          pass
        try:
          await user.join_chat("TeamSEMO")
        except:
          pass
        try:
          await user.join_chat("SEMOMusic")
        except:
          pass
        try:
          await user.join_chat("Dev_Mostafa")
        except:
          pass
        try:
          await user.join_chat("BarCinder")
        except:
          pass
        try:
          await user.join_chat("CH_SEMO")
        except:
          pass
...
["حول السورس", "استخراج جلسه"]],resize_keyboard=True)
    await message.reply_text(f"**👋🏻 ꒐ أهلاً بك {message.from_user.mention}  💎 .\n🤖 ꒐ في صانع ميوزك النجوم  💎 .**", reply_markup=kep,quote=True)
    
@app.on_message(filters.command(["حول السورس"], ""))
async def source(c: Client, m):
  mostafa ="""
╭──── • ◈ • ────╮
么 [𝒔𝒖𝒐𝒓𝒄𝒆 𝒆𝒍𝒏𝒈𝒐𝒐𝒎](t.me/SOURCE_ELNGOM) 💎 .
么  [𝒅𝒆𝒗 𝒔𝒆𝒎𝒐](t.me/A_q_lp) 💎 .
么  [𝒆𝒍𝒔𝒖𝒍𝒕𝒂𝒏](t.me/KAATARSU) 💎 .
╰──── • ◈ • ────╯
🚦 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼  💎 .
"""
  await m.reply_video("https://t.me/WORLED_SEMO/338",caption=mostafa,quote=True)
